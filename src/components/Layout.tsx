    ...
    <div className="flex h-full min-h-screen w-full bg-bg text-text-primary">
      ...
      <div className="flex w-full flex-col pl-[240px]">
        <header className="sticky top-0 z-10 flex h-14 items-center gap-4 border-b border-border bg-bg/85 px-6 backdrop-blur">
          <div className="relative w-[420px] max-w-[40%]">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted" />
            <input
              type="text"
              placeholder="Buscar SAs, workflows, motores, agentes…"
              className="h-9 w-full rounded-md border border-border bg-surface pl-9 pr-3 text-[13px] placeholder:text-text-muted focus:border-border-strong focus:outline-none"
            />
            <kbd className="absolute right-2 top-1/2 -translate-y-1/2 rounded border border-border bg-bg px-1.5 py-0.5 text-[10px] text-text-muted">
              ⌘K
            </kbd>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            <button className="flex h-9 items-center gap-1.5 rounded-md border border-border bg-surface px-3 text-[12px] text-text-secondary hover:text-text-primary">
              ...
              <span className="text-text-primary">prod</span>
              <ChevronDown className="h-3.5 w-3.5" />
            </button>
            <button className="relative flex h-9 w-9 items-center justify-center rounded-md border border-border bg-surface text-text-secondary hover:text-text-primary">
